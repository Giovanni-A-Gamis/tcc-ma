...
export default function Step6Memoria3({ navigation, route }) {
  const [selected, setSelected] = useState(null);
  const formData = route.params?.formData || {};

  const opcoes = [
    { texto: "Nunca tenho rotina organizada", pontos: 40 },
    { texto: "Raramente tenho rotina organizada", pontos: 80 },
    { texto: "Na maioria dos dias mantenho rotina organizada", pontos: 120 },
    { texto: "Sempre tenho rotina organizada", pontos: 160 },
  ];
  ...
  const next = () => {
    ...
    navigation.navigate('Step7Memoria4', {
      formData: { 
        ...formData,
        nivel_memoria: novoNivel,
        perguntas: [
          ...(formData.perguntas || []),
          { pergunta_id: 3, resposta: opcoes[selected].texto }
        ]
      }
    });
  };
    ...
    <ImageBackground source={fundo2} resizeMode="cover" style={styles.background}>
      <ScrollView style={styles.container2} contentContainerStyle={styles.content}>
        <Text style={styles.titulo2}>
          VOCÊ COSTUMA MANTER UMA ROTINA DIÁRIA ORGANIZADA?
        </Text>
